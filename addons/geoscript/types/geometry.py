...
class Geometry(AbstractSocket):
    ...
    def move_vertices(
        ...
        tree, node, layer = AbstractSocket.add_linked_node(
            arguments, "GeometryNodeSetPosition"
        )
    ...
    def set_id(self, id: Scalar | int, selection: Boolean = None):
        ...
        tree, node, layer = AbstractSocket.add_linked_node(
            arguments, "GeometryNodeSetID"
        )
        return Geometry(tree, node.outputs[0], layer)

    # "Geometry Proximity" in Blender:
    def __get_closest(
        self, target_element: str = "POINTS", source_position: Vector3 = None
    ):
        tree, node, layer = self.new_node(
            [self, source_position], "GeometryNodeProximity"
        )
    ...
    def transform(self, translation: Vector3, rotation: Vector3, scale: Vector3):
        ...
        tree, node, layer = AbstractSocket.add_linked_node(
            arguments, "GeometryNodeTransform"
        )
        return Geometry(tree, node.outputs[0], layer)
    ...
    def separate_geometry(self, selection: Boolean, domain: str = "POINT"):
        ...
        tree, node, layer = AbstractSocket.add_linked_node(
            arguments, "GeometryNodeSeparateGeometry"
        )

        selected = Geometry(tree, node.outputs[0], layer)
    ...
    def __get_component(self, index: int):
        if not hasattr(self, "_components_node"):
            t, node, l = AbstractSocket.add_linked_node(
    ...
    def merge_all_by_distance(self, selection: Boolean, merge_distance: Scalar):
        ...
        tree, node, layer = AbstractSocket.add_linked_node(
            arguments, "GeometryNodeMergeByDistance"
        )
        node.mode = "ALL"
    ...
    def to_instances(self):
        ...
        """
        tree, node, layer = AbstractSocket.add_linked_node(
            [self], "GeometryNodeGeometryToInstance"
        )
