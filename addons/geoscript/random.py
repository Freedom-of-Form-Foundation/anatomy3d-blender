...
def rand_float(
        ...
        max: Scalar | float,
        id: Scalar | int,
        seed: Scalar | int) -> Scalar:
    node_tree, node, layer = AbstractSocket.add_linked_node(
        [None, None, min, max, None, None, None, id, seed],
...
def rand_int(
        ...
        seed: Scalar | int) -> Scalar:
    node_tree, node, layer = AbstractSocket.add_linked_node(
        [None, None, None, None, min, max, None, id, seed],
        'FunctionNodeRandomValue')
...
def rand_vector(
        ...
        seed: Scalar | int) -> Scalar:
    node_tree, node, layer = AbstractSocket.add_linked_node(
        [min, max, None, None, None, None, None, id, seed],
        'FunctionNodeRandomValue')
    
    node.data_type = 'FLOAT_VECTOR'
    
    return Vector(node_tree, node.outputs[0], layer)
